...
def calculate_correlation_matrix(selected_funds_map):
    ...
    # Relax strict alignment to allow partial overlaps (pairwise correlation)
    combined = pd.DataFrame(frames)

    if combined.empty: return pd.DataFrame(), []

    # Calculate Correlation (pandas corr() handles pairwise NaNs automatically)
...
def run_crisis_audit(selected_funds_map):
    ...
    # Align all series (Allow partial history for portfolio simulation)
    combined_ret = pd.DataFrame({name: df['ret'] for name, df in frames.items()})
    if not combined_ret.empty:
        # Create portfolio return (mean of returns) - ignores NaNs for missing funds
